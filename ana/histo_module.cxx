...
public:
   ...
   void BeginRun(TARunInfo* runinfo)
   {
      ...
      h_aw_padcol_nocoinc = new TH2D("h_aw_padcol_nocoinc", "anode pad no coincidences;anode;pad sector", naw, 0, naw, nps, 0, nps);
      ...
      h_times_aw_p = new TH2D("h_times_aw_p", "pad / anode times;t_a;t_p",4000,-2000.,2000.,4000,-2000.,2000.);
      h_times_aw_p->SetMarkerStyle(8);
      h_times_aw_p->SetMarkerColor(kBlack);
      ...
      hoccaw = new TH1D("hoccaw","AW occupancy;AW",TPCBase::TPCBaseInstance()->GetNumberOfAnodeWires(),
                        0.,double(TPCBase::TPCBaseInstance()->GetNumberOfAnodeWires()));
      hoccawpos = new TH1D("hoccawpos","AW pos occupancy;#phi [deg]",TPCBase::TPCBaseInstance()->GetNumberOfAnodeWires(),
                           0.,360.);

      hoccpd = new TH2D("hoccpd","PAD occupancy;PAD",TPCBase::TPCBaseInstance()->GetNumberPadsRow(),
                        0.,double(TPCBase::TPCBaseInstance()->GetNumberPadsRow()),
                        TPCBase::TPCBaseInstance()->GetNumberPadsColumn(),
                        0.,double(TPCBase::TPCBaseInstance()->GetNumberPadsColumn()) );
      hoccpdpos = new TH2D("hoccpdpos","PAD pos occupancy;#phi [deg]",TPCBase::TPCBaseInstance()->GetNumberPadsRow(),
                           0.,360.,
                           TPCBase::TPCBaseInstance()->GetNumberPadsColumn(),
                           -TPCBase::TPCBaseInstance()->GetHalfLengthZ(),TPCBase::TPCBaseInstance()->GetHalfLengthZ());
   ...
   TAFlowEvent* AnalyzeFlowEvent(TARunInfo* runinfo, TAFlags* flags, TAFlowEvent* flow)
   {
            ...
            for(auto sp: SigFlow->pdSig)
               {
                  ...
                  if( fabs(sa.t-sp.t) < 16. )
                     h_aw_padcol->Fill(sa.i, sp.sec);

                  h_aw_padcol_nocoinc->Fill(sa.i, sp.sec);
      ...
      for(unsigned int i = 0; i < anodes.size(); i++)
         {
            ...
            hoccaw->Fill(anodes[i].i);
            double r, phi;
            TPCBase::TPCBaseInstance()->GetAnodePosition(anodes[i].i, r, phi, true);
            hoccawpos->Fill(phi*TMath::RadToDeg());
      ...
      for(unsigned int i= 0; i < pads.size(); i++)
         {
            hoccpd->Fill(pads[i].sec,pads[i].i);
            double z,phi;
            TPCBase::TPCBaseInstance()->GetPadPosition(pads[i].i, pads[i].sec, z, phi);
            hoccpdpos->Fill(phi*TMath::RadToDeg(),z);
