...
TAFlags *Aged::ShowEvent(AgEvent *age, AgAnalysisFlow *anaFlow, AgSignalsFlow *sigFlow, AgBarEventFlow *barFlow,
   ...
   if (data) {
      ...
      if (barFlow) {
         ...
         if (bars.size()) {
            ...
            double MeanTDC          = 0.;
            int    GoodTDC          = 0;
            memset(data->barpoints.bar_info, 0, num * sizeof(BarInfo));
            ...
            data->barpoints.meantdc = MeanTDC / double(GoodTDC);
         }
      }
    ...
    //   data->sigFlow = sigFlow;
      data->AWwf = &sigFlow->AWwf;
      data->PADwf = &sigFlow->PADwf;
    //   data->age     = age;
...
TAFlags *Aged::ShowEvent(TStoreEvent &evt, const std::vector<TBarHit *> &bars, const std::vector<ALPHAg::wf_ref> &AWwf,
                      const std::vector<ALPHAg::wf_ref> &PADwf, long runNo, TAFlags *flags)
{
   ImageData *data = fData;
