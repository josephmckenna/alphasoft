...
//---------------------------------------------------------------------------
// PDeconvWindow constructor
//
PDeconvWindow::PDeconvWindow(ImageData *data)
           : PImageWindow(data), mLastNum(-1), mChanMask(0)
{
    ...
    n = 0;
    XtSetArg(wargs[n], XmNtitle, "DeconvWaveforms"); ++n;
...
void PDeconvWindow::Listen(int message, void *message_data)
{
    switch (message) {
...
void PDeconvWindow::DoMenuCommand(int anID)
{
    switch (anID) {
...
void PDeconvWindow::ScrollValueChanged(EScrollBar bar, int value)
{
    switch (bar) {
        case kScrollRight: {
...
void PDeconvWindow::SetChannels(int chan_mask)
{
    int       n, i;
    Arg       wargs[20];
//    ImageData *data = GetData();
...
void PDeconvWindow::UpdateSelf()
{
    ...
    if (IsDirty() & (kDirtyEvent | kDirtyAll)) {
        ...
        if (hit_num >= 0) {
            ...
            if (!sigFlow->AWwf_deconv)
               printf("No AWwf in flow\n");
            else
               for (auto it=sigFlow->AWwf_deconv->begin(); it!=sigFlow->AWwf_deconv->end(); ++it) {
                   if (it->i == hi->wire) {
                       wave[kWireHist] = (void *)it->wf;
                       break;
                   }
               }
            if (!sigFlow->PADwf_deconv)
               printf("No AWwf in flow\n");
            else
               for (auto it=sigFlow->PADwf_deconv->begin(); it!=sigFlow->PADwf_deconv->end(); ++it) {
