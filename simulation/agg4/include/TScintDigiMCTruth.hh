...
#ifndef __TScintDigiMCTruth__
#define __TScintDigiMCTruth__ 1

#include <TObject.h>

class TScintDigiMCTruth: public TObject
{
private:
  int fID;  // G4 track ID
  int fPDG; // PDG code
...
public:
  TScintDigiMCTruth() {};
  TScintDigiMCTruth(int, int, double, double);
  //TScintDigiMCTruth(int, int, double, double, double, double);
  TScintDigiMCTruth(int, int, double, double, double);
...
//   inline double GetSigmaZ() const {return fSigmaZ;}
  ...
  bool IsSameDigi(TScintDigiMCTruth*);
  void PrintChannel();
