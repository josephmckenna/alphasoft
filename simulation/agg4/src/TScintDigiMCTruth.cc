...
TScintDigiMCTruth::TScintDigiMCTruth(int id, int pdg, double phi, double t):
                    fID(id),
                    fPDG(pdg),
					ft(t),
                    ft_in(nan("")),
...
// TScintDigiMCTruth::TScintDigiMCTruth(int id, int pdg,
// 		       double phi, double t, 
...
void TScintDigiMCTruth::PrintChannel()
{
  std::cout<<"*** TScintDigiMCTruth ***   "<<fID<<" "<<fPDG<<std::endl;
  std::cout<<"ch: "<<fBar
	   <<" --  Mutliplicity "<<fMultiplicity<<std::endl;
  std::cout<<"pos = ("<<fCentreR<<", "<<fPos<<")"<<std::endl;
}

ClassImp(TScintDigiMCTruth)
