...
int Alpha16Packet::PacketVersion(const void*ptr, int /*bklen*/)
{
   return getUint8(ptr, 1);
}

uint32_t Alpha16Packet::PacketTimestamp(const void*ptr, int /*bklen*/)
{
   return getUint32(ptr, 18);
}

int Alpha16Packet::PacketChannel(const void*ptr, int /*bklen*/)
{
   int chanX = getUint8(ptr, 27);
...
Alpha16Channel* Unpack(const char* bankname, int module, const Alpha16Packet* p, const void* bkptr, int /*bklen*/)
{
...
void Alpha16Asm::CheckEvent(Alpha16Event* e)
{
   ...
   for (unsigned i=0; i<e->hits.size(); i++) {
      ...
      int num_samples = e->hits[i]->adc_samples.size();
