...
void primary_pbar_origin() {
    ...
    corigin->Divide(2,2);
    corigin->cd(1);
        TH2D *h_origin_XY = new TH2D("h_origin_XY","Primary pbar origin X vs Y", 100, x_origin_min, x_origin_max, 100, y_origin_min, y_origin_max);
        tree->Draw("fVoy:fVox>>h_origin_XY");
        h_origin_XY->SetXTitle("cm"); h_origin_XY->SetYTitle("cm"); 
        h_origin_XY->Draw("COLZ");
    corigin->cd(2);
        TH1D *h_origin_Z = new TH1D("h_origin_Z", "Primary pbar origin Z", 100, z_origin_min, z_origin_max);
        tree->Draw("fVoz>>h_origin_Z");
        h_origin_Z->SetXTitle("cm");
        h_origin_Z->Draw("");
    corigin->cd(3);
        TH2D *h_origin_RvsZ = new TH2D("h_origin_RvsZ","Primary pbar origin R vs Z", 100, z_origin_min, z_origin_max, 100, 0., x_origin_max*1.1);
...
void primary_pbar_decay() {
    ...
    cdecay->Divide(2,2);
    cdecay->cd(1);
        TH2D *h_decay_XY = new TH2D("h_decay_XY","Primary pbar decay X vs Y", 100, x_decay_min*1.1, x_decay_max*1.1, 100, y_decay_min*1.1, y_decay_max*1.1);
        tree->Draw("fVdy:fVdx>>h_decay_XY");
        h_decay_XY->SetXTitle("cm"); h_decay_XY->SetYTitle("cm"); 
        h_decay_XY->Draw("COLZ");
    cdecay->cd(2);
        TH1D *h_decay_Z = new TH1D("h_decay_Z", "Primary pbar decay Z", 100, z_decay_min, z_decay_max);
        tree->Draw("fVdz>>h_decay_Z");
        h_decay_Z->SetXTitle("cm");
        h_decay_Z->Draw("");
    cdecay->cd(3);
        TH2D *h_decay_RvsZ = new TH2D("h_decay_RvsZ","Primary pbar decay R vs Z", 100, z_decay_min, z_decay_max, 100, 0., x_decay_max*1.1);
