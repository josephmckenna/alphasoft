...
Match::Match(const AnaSettings *ana_set, bool mt)
   : fTrace(false), fDebug(false), fMT(mt), ana_settings(ana_set),
     ...
     padThr(ana_settings->GetDouble(
        "DeconvModule", "PADthr")) // This DeconvModule setting is also needed here, for wire-dependent threshold
{
   // std::cout<<"Match::Loading AnaSettings from json"<<std::endl;

   if (fMT) CentreOfGravityFunction = 2;
   ...
   if (fMT) {
      if (fTrace) std::cout << "Match::Match MT-mode: selecting thread-safe CentreOfGravity_blobs" << std::endl;
   ...
   if (CentreOfGravityFunction <= 0) {
      ...
      std::cout << "Match::Match(json) Using CentreOfGravity case " << CentreOfGravityFunction << ": "
                << CentreOfGravity << std::endl;
}
...
void Match::Setup(TFile *OutputFile)
{
   if (diagnostic) {
      ...
      if (!hcognpeaks)
         hcognpeaks =
            new TH1D("hcognpeaks", "cCombPads CoG - Number of Avals", int(maxPadGroups + 1.), 0., maxPadGroups + 1.);
      // if (!hcognpeaksrms)
      //   hcognpeaksrms = new TH2D("hcognpeaksrms","CombPads CoG - Number of Avals vs RMS", 500, 0.,
      //   50,int(maxPadGroups+1.),
      // 			       0.,maxPadGroups+1.);
      // if (!hcognpeakswidth)
      //   hcognpeakswidth = new TH2D("hcognpeakswidth","CombPads CoG - Number of Avals vs width", 20, 0.,
      //   20,int(maxPadGroups+1.),
      // 				 0.,maxPadGroups+1.);
      if (!hcogsigma) hcogsigma = new TH1D("hcogsigma", "CombPads CoG - Sigma Charge Induced;[mm]", 700, 0., 70.);
      if (!hcogerr) hcogerr = new TH1D("hcogerr", "CombPads CoG - Error on Mean;[mm]", 2000, 0., 20.);

      if (!hcogpadssigma)
         hcogpadssigma =
            new TH2D("hcogpadssigma", "CombPads CoG - Pad Index Vs. Sigma Charge Induced;pad index;#sigma [mm]",
                     32 * 576, 0., 32. * 576., 1000, 0., 140.);
      if (!hcogpadsamp)
         hcogpadsamp =
            new TH2D("hcogpadsamp", "CombPads CoG - Pad Index Vs. Amplitude Charge Induced;pad index;Amplitude [a.u.]",
                     32 * 576, 0., 32. * 576., 1000, 0., 4000.);
      if (!hcogpadsint)
         hcogpadsint =
            new TH2D("hcogpadsint", "CombPads CoG - Pad Index Vs. Integral Charge Induced;pad index;Tot. Charge [a.u.]",
                     32 * 576, 0., 32. * 576., 1000, 0., 10000.);
      if (!hcogpadsampamp)
         hcogpadsampamp =
            new TH2D("hcogpadsampamp",
                     "CombPads CoG - Gaussian fit amplitude Vs. Max. Signal height;max. height;Gauss Amplitude", 1000,
                     0., 4000., 1000, 0., 4000.);
      //  hsig = new TH1D("hpadRowSig","sigma of pad combination fit",1000,0,50);
      if (!htimecog) htimecog = new TH1D("htimecog", "Timing of Cog;Time [us]", 1000, 0., 10000.);
      if (!htimeblobs) htimeblobs = new TH1D("htimeblobs", "Timing of Blob Finding;Time [us]", 1000, 0., 10000.);
      if (!htimefit) htimefit = new TH1D("htimefit", "Timing of Fit;Time [us]", 1000, 0., 10000.);
   }
}

std::pair<std::set<short>, std::vector<std::vector<ALPHAg::TPadSignal>>> Match::PartitionBySector(
...
std::vector<std::vector<ALPHAg::TPadSignal>> Match::PartitionByTime(std::vector<ALPHAg::TPadSignal> &sig)
{
   if (fDebug) std::cout << "Match::PartitionByTime  " << sig.size() << std::endl;
   std::multiset<ALPHAg::TPadSignal, ALPHAg::signal::timeorder> sig_bytime(sig.begin(), sig.end());
   double                                                       temp = -999999.;
...
std::vector<std::vector<ALPHAg::TPadSignal>> Match::CombPads(std::vector<ALPHAg::TPadSignal> &padsignals)
{
   ...
   if (int(padsignals.size()) > fNpadsCut) {
      if (fTrace)
         std::cout << "Match::CombPads number of pads signals " << padsignals.size() << " exceeds its limit "
                   << fNpadsCut << std::endl;
      comb.resize(0);
      return comb;
   }
   ...
   for (auto isec = secs.begin(); isec != secs.end(); ++isec) {
      ...
      if (fDebug)
         std::cout << "Match::CombPads sec: " << sector << " = sector: " << pad_bysec[sector].at(0).sec
                   << " size: " << pad_bysec[sector].size() << std::endl;
      // combine pads in the same time slice only
      std::vector<std::vector<ALPHAg::TPadSignal>> pad_bytime = PartitionByTime(pad_bysec[sector]);
   ...
   secs.clear();
   pad_bysec.clear();
   if (fTrace) std::cout << "Match::CombPads # of teeth: " << comb.size() << std::endl;
   return comb;
}

std::vector<ALPHAg::TPadSignal> Match::CombineAPad(std::vector<std::vector<ALPHAg::TPadSignal>> &comb,
                                                   std::vector<ALPHAg::TPadSignal> &CombinedPads, size_t PadNo)
{

   if (PadNo > comb.size()) return CombinedPads;
...
std::vector<ALPHAg::TPadSignal> Match::CombinePads(std::vector<std::vector<ALPHAg::TPadSignal>> &comb)
{
   ...
   case 1: {
      for (auto sigv = comb.begin(); sigv != comb.end(); ++sigv) {
         ...
         if (fTrace)
            std::cout << "Match::CombinePads Time taken CentreOfGravity: " << duration.count() << " us" << std::endl;
   ...
   case 2: {
      ...
      for (unsigned i = 0; i < comb.size(); ++i) {
         cogthread.push_back(
            std::thread(&Match::CentreOfGravity_blobs, this, std::ref(comb.at(i)), std::ref(CombinedPads)));
      }
      ...
      if (fTrace) {
         std::cout << "Match::CombinePads Time taken CentreOfGravity_blobs: " << cogdura.count() << " us" << std::endl;
      }
      break;
   }
   }
...
void Match::CentreOfGravity(std::vector<ALPHAg::TPadSignal> &vsig, std::vector<ALPHAg::TPadSignal> &CombinedPads)
{
   ...
   if (fDebug) std::cout << "Match::CentreOfGravity nfound: " << nfound << " @ t: " << time << std::endl;
   if (nfound > 1 && hh->GetRMS() < spectrum_width_min) {
      nfound = 1;
      if (fDebug) std::cout << "\tRMS is small: " << hh->GetRMS() << " set nfound to 1" << std::endl;
   }
   ...
   for (int i = 0; i < nfound; ++i) {
      ...
      if (r == 0) // it's good
      {
         ...
         if (err < padFitErrThres && fabs(sigma - padSigma) / padSigma < padSigmaD)
         // if( err < padFitErrThres && sigma > 0. )
         {
            ...
            if (fDebug)
               std::cout << "Combination Found! s: " << col << " i: " << row << " t: " << time << " a: " << amp
                         << " z: " << pos << " err: " << err << std::endl;
         } else // fit is crazy
         {
            if (fTrace)
               std::cout << "Combination NOT found... position error: " << err << " or sigma: " << sigma << std::endl;
#ifdef RESCUE_FIT
            stat = false;
#endif
         }
         ...
         if (fTrace) std::cout << "\tFit Not valid with status: " << r << std::endl;
#ifdef RESCUE_FIT
         stat = false;
#endif
      }
      ...
      if (!stat) {
         ...
         if (tot > 0.) {
            ...
            if (fDebug)
               std::cout << "at last Found! s: " << col << " i: " << index << " t: " << time << " a: " << amp
                         << " z: " << pos << std::endl;
            stat = true;
         } else {
            if (fTrace) std::cout << "Failed last combination resort" << std::endl;
         }
      }
#endif
   } // wizard peak finding failed
   delete hh;
   if (fTrace) std::cout << "-------------------------------" << std::endl;
   manalzer_global_mtx->unlock();
}

// TH1-independent method to find peaks in pad charge distribution
std::vector<std::pair<double, double>> Match::FindBlobs(const std::vector<ALPHAg::TPadSignal> &sigs, int ifirst,
                                                        int ilast)
{
...
void Match::CentreOfGravity_blobs(std::vector<ALPHAg::TPadSignal> &vsig, std::vector<ALPHAg::TPadSignal> &CombinedPads)
{
   ...
   if (fTrace)
      std::cout << "MatchModule::CentreOfGravity_blobs nfound: " << nfound << " @ t: " << time << " in sec: " << col
                << std::endl;
   ...
   for (int i = 0; i < nfound; ++i) {
      ...
      for (int j = 0; j < i; j++) {
         if (abs(blobs[i].first - blobs[j].first) < goodDist)
            if (blobs[i].second < grassCut * blobs[j].second) grass = true;
      }
      if (!grass) {
         if (fDebug) std::cout << blobs[i].first << '\t';
   ...
   if (fDebug)
      std::cout << "Match::MatchModule::CentreOfGravity_blobs nfound after grass cut: " << nfound << " @ t: " << time
                << " in sec: " << col << std::endl;
      ...
      for (int i = 0; i < nfound; ++i) {
         ...
         if (err < padFitErrThres && fabs(sigma - padSigma) / padSigma < padSigmaD) {
            if (fabs(pos) < ALPHAg::_halflength) {
               if (fMT) manalzer_global_mtx->lock();
               // create new signal with combined pads
               CombinedPads.emplace_back(ALPHAg::electrode(col, row), time, amp, amp_err, pos, err);
               if (fMT) manalzer_global_mtx->unlock();
               // signal pad_cog( col, row, time, amp, amp_err, pos, err );
               // padcog.push_back(pad_cog);
               ++nPositions;
               if (fDebug)
                  std::cout << "CoG_blobs Combination Found! s: " << col << " i: " << row << " t: " << time
                            << " a: " << amp << " z: " << pos << " err: " << err << std::endl;
            } else {
               if (fDebug)
                  std::cout << "CoG_blobs Bad Combination Found! (z outside TPC) s: " << col << " i: " << row
                            << " t: " << time << " a: " << amp << " z: " << pos << " err: " << err << std::endl;
            }
         } else // fit is crazy
         {
            if (fTrace)
...
std::vector<std::pair<ALPHAg::TWireSignal, ALPHAg::TPadSignal>> Match::MatchElectrodes(
   std::vector<ALPHAg::TWireSignal> &awsignals, std::vector<ALPHAg::TPadSignal> &CombinedPads)
{
   std::multiset<ALPHAg::TWireSignal, ALPHAg::signal::timeorder> aw_bytime(awsignals.begin(), awsignals.end());
   std::multiset<ALPHAg::TPadSignal, ALPHAg::signal::timeorder>  pad_bytime(CombinedPads.begin(), CombinedPads.end());
   ...
   for (auto iaw = aw_bytime.begin(); iaw != aw_bytime.end(); ++iaw) {
      ...
      if (fTrace)
         std::cout << "Match::Match aw: " << iaw->idx << " t: " << iaw->t << " pad sector: " << sector << std::endl;
   ...
   if (int(spacepoints.size()) != Nmatch)
      std::cerr << "Match::MatchElectrodes ERROR: number of matches differs from number of spacepoints: "
                << spacepoints.size() << std::endl;
   return spacepoints;
}

std::vector<std::pair<ALPHAg::TWireSignal, ALPHAg::TPadSignal>> Match::FakePads(
   std::vector<ALPHAg::TWireSignal> &awsignals)
{
   std::multiset<ALPHAg::TWireSignal, ALPHAg::signal::timeorder>   aw_bytime(awsignals.begin(), awsignals.end());
   std::vector<std::pair<ALPHAg::TWireSignal, ALPHAg::TPadSignal>> spacepoints;
   int                                                             Nmatch = 0;
   ...
   if (int(spacepoints.size()) != Nmatch)
      std::cerr << "Match::FakePads ERROR: number of matches differs from number of spacepoints: " << spacepoints.size()
                << std::endl;
   if (fTrace) std::cout << "Match::FakePads Number of Matches: " << Nmatch << std::endl;
   return spacepoints;
}

void Match::SortPointsAW(
   const std::pair<double, int> &pos, std::vector<std::pair<ALPHAg::TWireSignal, ALPHAg::TPadSignal> *> &vec,
   std::map<int, std::vector<std::pair<ALPHAg::TWireSignal, ALPHAg::TPadSignal> *>, std::greater<int>> &spaw)
{
   for (auto &s : vec) {
      if (1)
         std::cout << "\ttime: " << pos.first << " row: " << pos.second << " aw: " << s->first.idx
                   << " amp: " << s->first.height << "   (" << s->first.t << ", " << s->second.idx << ")" << std::endl;
   ...
   std::map<int, std::vector<std::pair<ALPHAg::TWireSignal, ALPHAg::TPadSignal> *>> &                   merger)
{
   ...
   for (auto &msp : spaw) {
      ...
      for (auto &s : msp.second) {
         ...
         if (fTrace)
            std::cout << "\t" << m << " aw: " << s->first.idx << " amp: " << s->first.height << " phi: " << s->first.phi
                      << "   (" << s->first.t << ", " << s->second.idx << ", "
                      << ALPHAg::_anodepitch * (double(s->first.idx) + 0.5) << ") {" << s->first.idx % 8 << ", "
                      << s->first.idx / 8 << ", " << s->second.sec << "}" << std::endl;
         aw = s->first.idx;
   ...
   for (auto &msp : spaw) {
      for (auto &s : msp.second) {
         ...
         if (0)
            std::cout << "\t" << m << " aw: " << s->first.idx << " amp: " << s->first.height << " phi: " << s->first.phi
                      << "   (" << s->first.t << ", " << s->second.idx << ", "
                      << ALPHAg::_anodepitch * (double(s->first.idx) + 0.5) << ") " << std::endl;
         aw = s->first.idx;
...
uint Match::MergePoints(std::map<int, std::vector<std::pair<ALPHAg::TWireSignal, ALPHAg::TPadSignal> *>> &merger,
                        std::vector<std::pair<ALPHAg::TWireSignal, ALPHAg::TPadSignal>> &merged, uint &number_of_merged)
{
   uint np = 0;
   for (auto &mmm : merger) {
      double pos = 0., amp = 0.;
      double maxA = amp, amp2 = amp * amp;
      if (fTrace) std::cout << "===" << mmm.first << std::endl;
      np += mmm.second.size();
      uint j = 0, idx = j;
      int  wire = -1;
      for (auto &p : mmm.second) {
         double A = p->first.height, pphi = p->first.phi;
         if (0)
            std::cout << " aw: " << p->first.idx << " amp: " << p->first.height << " phi: " << p->first.phi << "   ("
                      << p->first.t << ", " << p->second.idx << ", "
                      << ALPHAg::_anodepitch * (double(p->first.idx) + 0.5) << ") " << std::endl;
         amp += A;
         amp2 += (A * A);
         pos += (pphi * A);
         if (A > maxA) {
      ...
      double phi = pos / amp, err = phi_err * sqrt(amp2) / amp, H = amp / double(mmm.second.size());
      if (fTrace)
         std::cout << "\tpnt: " << phi << " +/- " << err << " A: " << H << " # " << mmm.second.size()
                   << " wire: " << wire << " maxA: " << maxA << std::endl;
...
std::vector<std::pair<ALPHAg::TWireSignal, ALPHAg::TPadSignal>> Match::CombPoints(
   std::vector<std::pair<ALPHAg::TWireSignal, ALPHAg::TPadSignal>> &spacepoints)
{
   if (fTrace) std::cout << "Match::CombPoints() spacepoints size: " << spacepoints.size() << std::endl;
   ...
   if (fTrace) std::cout << "Match::CombPoints() comb size: " << combsp.size() << std::endl;
   uint                                                            n = 0;
   ...
   for (auto &k : combsp) {
      ...
      if (k.second.size() > 1) {
         if (fTrace)
            std::cout << "Match::CombPoints() vec size: " << k.second.size() << "\ttime: " << k.first.first
         ...
         if (0) std::cout << "Match::CombPoints() merger size: " << merger.size() << std::endl;

         uint np = MergePoints(merger, merged, m);
         if (np != k.second.size())
            std::cerr << "Match::CombPoints() ERROR tot merger size: " << np << " vec size: " << k.second.size()
                      << std::endl;
   ...
   if (n != spacepoints.size())
      std::cerr << "Match::CombPoints() ERROR total comb size: " << n << "spacepoints size: " << spacepoints.size()
                << std::endl;
   if ((n - merged.size()) != m)
      std::cerr << "Match::CombPoints() ERROR spacepoints merged diff size: " << n - merged.size() << "\t" << m
                << std::endl;

   spacepoints.assign(merged.begin(), merged.end());
   if (fDebug) {
      std::cout << "Match::CombPoints() spacepoints merged size: " << merged.size() << " (diff: " << m << ")"
                << std::endl;
      std::cout << "Match::CombPoints() spacepoints size (after merge): " << spacepoints.size() << std::endl;
   }
   if (fTrace) std::cout << "Match::CombPoints() " << spacepoints.size() << " found" << std::endl;
   return spacepoints;
}
