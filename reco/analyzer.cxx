...
{
  time = time;     // to remove unused parameter warnings
...
void HandleMidasEvent(TMidasEvent& event)
{
  ...
  if (eventId == 1)
    {
      Alpha16Event* e = UnpackAlpha16Event(gA16Evb, &event);
           // e->Print();
...
int ProcessMidasFile(TApplication*app,const char*fname)
{
  ...
  while (1)
	...
	break;
      //      char opt = 0;
      // if(steps == 0){
      //     if(i) cout << "Stopped after event " << i-1 << endl;
      //     cout << "Analyze how many events? (0 to stop, -1 for all)" << endl;
      //     std::cin >> steps;
      //     // if(!std::cin) std::cin >> opt;
      //     // cout << ">>" << opt << "<<" << endl;
      // }
      //      if(steps == 0) break;

      // if(i == 8){
      //     TString fn = TString::Format("deconv_%02d.gif+", i);
      //     cout << "Saving to " << fn.Data() << endl;
      //     agan->SaveDeconv(fn.Data());
      // }
      // if(i == 450){
      //     agan->SetPlotTracks();
      //     //          agan->SetShowDeconv(2);
      //     //          agan->SaveGIFtracks("decTracks95.gif",100);
      // }
      int eventId = event.GetEventId();
    ...
    : TGMainFrame(w,s1,s2)
{
   ...
   TGTextButton *Ter = new TGTextButton(this,"Next Event");
   ULong_t yellow;
   gClient->GetColorByName("yellow", yellow);
   Ter->ChangeBackground(yellow);
   AddFrame(Ter, new TGLayoutHints(kLHintsTop | kLHintsExpandX,
				    2, 0, 2, 2));
   Ter->Connect("Pressed()","TApplication",xapp, "Terminate()");
...
int main(int argc, char *argv[])
{
   ...
   TApplication app("rootana", &argc, argv);
   xapp=&app;
   ...
   if(!agan->SetGas(0.28)) return -1;
   if(!agan->SetB(0)) return -2;
   // agan->SetShowDeconv();
   // agan->SetPhi0(phi_0);
   // agan->SetT0(tdelay);
   if( forceEnableGraphics )
     agan->SetPlotTracks();

   // TGraph roft;
   // roft.SetLineColor(kRed);
   // roft.SetTitle("Garfield");
   // for(int i = 0; i < hRofT_straight.GetNbinsX(); i++){
   //     double t = hRofT_straight.GetXaxis()->GetBinCenter(i);
   //     double r = spoints.lookup.GetRadius(t);
   //     int n = roft.GetN();
   //     if(r > 0) roft.SetPoint(n, t, r);
   //     //               roft.SetPointError(n, 0, spoints.lookup.GetdR(t));
   // }
   ...
   cout << "Discarded " << incomplete << " incomplete events." << endl;
   // TFile of("output.root","RECREATE");
   // hFirstTime.Write();
   // hAnodeHits.Write();
   // hRofT_straight.Write();
   // of.Write();
   // of.Close();
   //   app->Run(kTRUE);
